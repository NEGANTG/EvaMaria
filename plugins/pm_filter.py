...
async def auto_filter(client, message):
    ...
    if 2 < len(message.text) < 100:
        ...
        if imdb and imdb.get('poster'):
            await message.reply_photo(photo=imdb.get('poster'), caption=f"🏷 Title: {imdb.get('title')}\n🎭 Genres: {imdb.get('genres')}\n🌟 Rating: {imdb.get('rating')} / 10\n📆 Year: {imdb.get('year')}\n🖋 StoryLine: <code>{imdb.get('plot')} </code>", reply_markup=InlineKeyboardMarkup(btn))
        elif imdb:
            await message.reply_text(f"🏷 Title: {imdb.get('title')}\n🎭 Genres: {imdb.get('genres')}\n🌟 Rating: {imdb.get('rating')} / 10\n📆 Year: {imdb.get('year')}\n🖋 StoryLine: <code>{imdb.get('plot')} </code>", reply_markup=InlineKeyboardMarkup(btn))
        else:
            await message.reply_text(f"Here is What I Found In My Database For Your Query {search} ‌‌‌‌‎ ", reply_markup=InlineKeyboardMarkup(btn))
